...
search_forwards <- function(sleep_index, noon_days, data){
...
search_backwards <- function(sleep_index, noon_days, data){
  curr_position <- sleep_index - 1
...
for(i in 1:length(paths)){
  ...
  noon_days <- c(1)
  for(i in 1:nrow(data)){
    if(as.numeric(difftime(data[i, 1], curr_day), units = "days") > 1) {
...
for(i in 1:(length(noon_days) - 1)){
  ...
  for(j in 1:length(sleep_indices)){
    all_SLNW <- c(all_SLNW, search_backwards(sleep_indices[j], noon_days, data), sleep_indices[j], search_forwards(sleep_indices[j], noon_days, data))
  }
}
all_SLNW <- all_SLNW[!duplicated(all_SLNW)]
